...
static const char *const k_corpus[] = {
    ...
    "", "00", "0000", "000000",
    /* bare headers */
    "00A40000", "FFFFFFFF", "00000000", "FFFFFF00",
    /* huge Lc with no data */
    "00A40000FF", "00D60000FF", "00A4000080",
    /* Lc claiming more than present */
    "00A40000FF3F00", "00A400000A3F00", "00D60000FF0102",
    /* huge Le */
    "00B0000000", "00B00000FF", "00A4000002 3F00 00",
    /* extended-length shapes */
    "00A400000000023F00", "00A40000000000", "00A4000000FFFF",
    "00B0000000000000", "00A400000001",
    /* invalid CLA / INS */
    "FFA40000", "80EE0000", "0CA40000", "10A40000", "01A40000",
    /* TLV: truncated, indefinite, oversized, deep nesting */
    "62", "6205", "620582", "62058201", "3080820138 0000",
    "80FF01", "808301 0000", "5F", "5F812D01AA",
    "62 04 62 02 62 00", "62 06 62 04 62 02 62 00",
...
static size_t gen_apdu_like(uint8_t *buf, size_t cap)
{
    ...
    case 1: actual = (lc > 0u) ? lc - 1u : 0u; break;/* one short   */
    case 2: actual = lc + 1u; break;                 /* one long    */
    default: actual = rnd_below(262u); break;        /* unrelated   */
    }
...
static size_t gen_tlv_like(uint8_t *buf, size_t cap)
{
    ...
    for (unsigned o = 0; o < objects && n + 4u < cap; o++) {
        ...
        case 1:
            ...
            if (n < cap) { buf[n++] = (uint8_t)claimed; }
            break;
        case 2:
            buf[n++] = 0x82u;
            claimed = rnd_below(0x10000u);
            if (n < cap) { buf[n++] = (uint8_t)(claimed >> 8); }
            if (n < cap) { buf[n++] = (uint8_t)(claimed & 0xFFu); }
            break;
        default:
        ...
        const uint32_t actual = (rnd_below(3u) == 0u) ? claimed
                                                      : rnd_below(64u);
        for (uint32_t i = 0; i < actual && n < cap; i++) {
            buf[n++] = (uint8_t)(rnd() & 0xFFu);
        }
...
{
    if (strcmp(name, "apdu") == 0)     { return scos_fuzz_apdu; }
    if (strcmp(name, "tlv") == 0)      { return scos_fuzz_tlv; }
...
int main(int argc, char **argv)
{
    ...
    unsigned long iterations = (argc >= 3) ? strtoul(argv[2], NULL, 0) : 20000ul;
    ...
    buf[0] = 0x00; buf[1] = 0xD6; buf[2] = 0x00; buf[3] = 0x00; buf[4] = 0xFF;
    for (unsigned i = 5; i < 260u; i++) { buf[i] = (uint8_t)i; }
    ...
    for (unsigned long it = 0; it < iterations; it++) {
        size_t n;
        switch (it % 3ul) {
        case 0:  n = gen_apdu_like(buf, sizeof(buf)); break;
        case 1:  n = gen_tlv_like(buf, sizeof(buf));  break;
        default: n = gen_random(buf, sizeof(buf));    break;
        }
        (void)target(buf, n);
    }
