...
TEST(case3_max_lc)
{
    ...
    for (unsigned i = 5; i < sizeof(buf); i++) { buf[i] = (uint8_t)i; }
...
TEST(case4_max_length)
{
    ...
    uint8_t buf[261];
    buf[0] = 0x00; buf[1] = 0xD6; buf[2] = 0x00; buf[3] = 0x00; buf[4] = 0xFF;
    for (unsigned i = 5; i < 260; i++) { buf[i] = 0xAA; }
    buf[260] = 0x05;
...
TEST(exhaustive_length_consistency)
{
    ...
    buf[0] = 0x00; buf[1] = 0xA4; buf[2] = 0x00; buf[3] = 0x00;
    ...
    for (unsigned lc = 1; lc <= 255u; lc++) {
        ...
        for (unsigned len = 6u; len <= 261u && len <= sizeof(buf); len++) {
            apdu_command c;
            const apdu_parse_status st =
                apdu_parse(buf, (uint16_t)len, &c);
    ...
    CHECK(ok_count > 400); /* sanity: the sweep really did exercise the OK path */
}
